export function TeachingSection() {
  return (
    <section className="bg-[linear-gradient(90deg,#fff6f1_0%,#f4d9df_100%)] px-6 py-12 text-center md:px-10 md:py-18">
      <p className="mb-2.5 text-[0.78rem] font-semibold uppercase tracking-[0.28em] text-[var(--color-muted)] md:mb-3 md:text-sm">
        Rooted in Tradition
      </p>

      <h2
        dir="rtl"
        className="mb-4 font-display text-[clamp(2rem,10vw,4rem)] text-[var(--color-burgundy)] md:mb-5"
      >
        נידה, חלה והדלקת הנר
      </h2>

      <p className="mx-auto max-w-6xl text-[0.95rem] leading-6 text-[var(--color-copy)] md:text-[1rem] md:leading-7">
        In the Mishnah, the first written record of Jewish wisdom passed down
        through generations, three sacred mitzvot are entrusted to Jewish
        women: niddah (mikveh), challah, and lighting Shabbat candles, each
        carrying the power to bring presence, holiness, and life into the
        world.
      </p>
    </section>
  );
}
